...
class RHD_VirtualPlayerState
{
	...
	{
		foreach (RHD_VirtualInventoryEntry entry : entries)
			if (entry && entry.m_sItemId == itemId && entry.m_bVirtual == virtualItem) return entry.m_iQuantity;
		return 0;
	}
	bool AddBaseItem(string itemId, string displayName, int quantity)
	{
		...
		RHD_VirtualInventoryEntry entry = new RHD_VirtualInventoryEntry(); entry.m_sItemId = itemId; entry.m_sDisplayName = displayName; entry.m_iQuantity = quantity; entry.m_bVirtual = false; m_aBaseInventory.Insert(entry); return true;
	}
	...
	protected bool RemoveItem(array<RHD_VirtualInventoryEntry> entries, string itemId, int quantity, bool virtualItem)
	{
		if (itemId.IsEmpty() || quantity <= 0) return false;
		foreach (RHD_VirtualInventoryEntry entry : entries)
		{
			if (!entry || entry.m_sItemId != itemId || entry.m_bVirtual != virtualItem || entry.m_iQuantity < quantity) continue;
			entry.m_iQuantity -= quantity; if (entry.m_iQuantity <= 0) entries.RemoveItem(entry); return true;
		}
		return false;
	}
	bool AddGarageVehicle(string vehicleId, string displayName) { if (vehicleId.IsEmpty()) return false; RHD_GarageVehicleEntry entry = new RHD_GarageVehicleEntry(); entry.m_sVehicleId = vehicleId; entry.m_sDisplayName = displayName; entry.m_bStored = true; m_aGarage.Insert(entry); return true; }
	bool AddProperty(string propertyId, string displayName, vector position, float radiusMeters, int price)
	{
		if (m_aProperties.Count() >= RHD_VirtualPlayerConfig.MAX_PROPERTIES || propertyId.IsEmpty() || radiusMeters <= 0 || radiusMeters > RHD_VirtualPlayerConfig.MAX_BUILD_RADIUS_M) return false;
		if (price < RHD_VirtualPlayerConfig.MIN_PROPERTY_PRICE || price > RHD_VirtualPlayerConfig.MAX_PROPERTY_PRICE) return false;
		RHD_PropertyEntry property = new RHD_PropertyEntry(); property.m_sPropertyId = propertyId; property.m_sDisplayName = displayName; property.m_vPosition = position; property.m_fBuildRadiusMeters = radiusMeters; property.m_iPurchasePrice = price; property.m_iStorageCapacity = 0; property.m_bOwned = true; m_aProperties.Insert(property); return true;
	}
	bool AddStorageObject(int propertyIndex, string storageId)
	{
		...
		RHD_PropertyEntry property = m_aProperties[propertyIndex]; if (!property || property.m_iStorageCapacity + RHD_VirtualPlayerConfig.STORAGE_PER_OBJECT > RHD_VirtualPlayerConfig.MAX_PROPERTY_STORAGE) return false;
		RHD_PropertyStorageEntry storage = new RHD_PropertyStorageEntry(); storage.m_sStorageId = storageId; storage.m_iCapacity = RHD_VirtualPlayerConfig.STORAGE_PER_OBJECT; property.m_aStorage.Insert(storage); property.m_iStorageCapacity += storage.m_iCapacity; return true;
	}
};
enum RHD_JobStage { TALK_TO_SOURCE, TALK_TO_CONTACT, DELIVER_ITEM, COMPLETE };
