class RHD_ATAKMenuUI : ChimeraMenuBase
{
	...
	override void OnMenuOpen()
	{
		...
		if (inputManager) inputManager.AddActionListener("MenuBack", EActionTrigger.DOWN, CloseMenu);
	}
	override void OnMenuClose()
	{
		InputManager inputManager = GetGame().GetInputManager();
		if (inputManager) inputManager.RemoveActionListener("MenuBack", EActionTrigger.DOWN, CloseMenu);
		super.OnMenuClose();
	}
	protected void OnCloseClicked() { CloseMenu(); }
	protected void CloseMenu(float value = 0, EActionTrigger reason = EActionTrigger.DOWN)
	{
		GetGame().GetMenuManager().CloseMenuByPreset(ChimeraMenuPreset.RHD_ATAK_Menu);
	}
	void RefreshStats()
	{
		Widget root = GetRootWidget();
		...
		if (money) money.SetText("$0");
		TextWidget content = TextWidget.Cast(root.FindAnyWidget(TEXT_CONTENT));
		if (content) content.SetText("Shop | Casino | Map | Stats | Handler");
	}
};
